...
class Ontology(object):
    ...
    def get_goidlist(self, geneIDList, domainList, ecList):
        resp = self._call('Ontology.get_goidlist',
                          [geneIDList, domainList, ecList])
        return resp[0]

    def get_go_description(self, goIDList):
        resp = self._call('Ontology.get_go_description',
                          [goIDList])
        return resp[0]

    def get_go_enrichment(self, geneIDList, domainList, ecList, type, ontologytype):
        resp = self._call('Ontology.get_go_enrichment',
                          [geneIDList, domainList, ecList, type, ontologytype])
        return resp[0]

    def get_go_annotation(self, geneIDList):
        resp = self._call('Ontology.get_go_annotation',
                          [geneIDList])
        return resp[0]

    def association_test(self, gene_list, ws_name, in_obj_id, out_obj_id, type, correction_method, cut_off):
        resp = self._call('Ontology.association_test',
                          [gene_list, ws_name, in_obj_id, out_obj_id, type, correction_method, cut_off])
        return resp[0]
