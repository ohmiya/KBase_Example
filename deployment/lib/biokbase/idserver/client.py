...
class IDServerAPI(object):
    ...
    def kbase_ids_to_external_ids(self, ids):
        resp = self._call('IDServerAPI.kbase_ids_to_external_ids',
                          [ids])
        return resp[0]

    def external_ids_to_kbase_ids(self, external_db, ext_ids):
        resp = self._call('IDServerAPI.external_ids_to_kbase_ids',
                          [external_db, ext_ids])
        return resp[0]

    def register_ids(self, prefix, db_name, ids):
        resp = self._call('IDServerAPI.register_ids',
                          [prefix, db_name, ids])
        return resp[0]

    def allocate_id_range(self, kbase_id_prefix, count):
        resp = self._call('IDServerAPI.allocate_id_range',
                          [kbase_id_prefix, count])
        return resp[0]

    def register_allocated_ids(self, prefix, db_name, assignments):
        self._call('IDServerAPI.register_allocated_ids',
                   [prefix, db_name, assignments])

    def get_identifier_prefix(self):
        resp = self._call('IDServerAPI.get_identifier_prefix',
                          [])
        return resp[0]
