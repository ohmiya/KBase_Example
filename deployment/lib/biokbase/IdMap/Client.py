...
class IdMap(object):
    ...
    def lookup_genome(self, s, type):
        resp = self._call('IdMap.lookup_genome',
                          [s, type])
        return resp[0]

    def lookup_features(self, genome_id, aliases, feature_type, source_db):
        resp = self._call('IdMap.lookup_features',
                          [genome_id, aliases, feature_type, source_db])
        return resp[0]

    def lookup_feature_synonyms(self, genome_id, feature_type):
        resp = self._call('IdMap.lookup_feature_synonyms',
                          [genome_id, feature_type])
        return resp[0]

    def longest_cds_from_locus(self, arg_1):
        resp = self._call('IdMap.longest_cds_from_locus',
                          [arg_1])
        return resp[0]

    def longest_cds_from_mrna(self, arg_1):
        resp = self._call('IdMap.longest_cds_from_mrna',
                          [arg_1])
        return resp[0]
