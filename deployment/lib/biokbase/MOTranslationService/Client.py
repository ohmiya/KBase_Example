...
class MOTranslation(object):
    ...
    def fids_to_moLocusIds(self, fids):
        resp = self._call('MOTranslation.fids_to_moLocusIds',
                          [fids])
        return resp[0]

    def proteins_to_moLocusIds(self, proteins):
        resp = self._call('MOTranslation.proteins_to_moLocusIds',
                          [proteins])
        return resp[0]

    def moLocusIds_to_fids(self, moLocusIds):
        resp = self._call('MOTranslation.moLocusIds_to_fids',
                          [moLocusIds])
        return resp[0]

    def moLocusIds_to_proteins(self, moLocusIds):
        resp = self._call('MOTranslation.moLocusIds_to_proteins',
                          [moLocusIds])
        return resp[0]

    def map_to_fid(self, query_sequences, genomeId):
        resp = self._call('MOTranslation.map_to_fid',
                          [query_sequences, genomeId])
        return resp

    def map_to_fid_fast(self, query_md5s, genomeId):
        resp = self._call('MOTranslation.map_to_fid_fast',
                          [query_md5s, genomeId])
        return resp

    def moLocusIds_to_fid_in_genome(self, moLocusIds, genomeId):
        resp = self._call('MOTranslation.moLocusIds_to_fid_in_genome',
                          [moLocusIds, genomeId])
        return resp

    def moLocusIds_to_fid_in_genome_fast(self, moLocusIds, genomeId):
        resp = self._call('MOTranslation.moLocusIds_to_fid_in_genome_fast',
                          [moLocusIds, genomeId])
        return resp

    def moTaxonomyId_to_genomes(self, moTaxonomyId):
        resp = self._call('MOTranslation.moTaxonomyId_to_genomes',
                          [moTaxonomyId])
        return resp[0]
