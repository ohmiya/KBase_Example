...
# Authentication: none
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function get_identifier_prefix (received $n, expecting 0)");
    }

    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
	method => "IDServerAPI.get_identifier_prefix",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method get_identifier_prefix",
					    status_line => $self->{client}->status_line,
					    method_name => 'get_identifier_prefix',
				       );
    }
...
sub version {
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method get_identifier_prefix",
            status_line => $self->{client}->status_line,
            method_name => 'get_identifier_prefix',
        );
    }
