package ProbabilisticAnnotationServer;
...
our %method_authentication = (
        ...
        'annotate' => 'required',
        'calculate' => 'required',
        'get_rxnprobs' => 'required',
        'get_probanno' => 'required',
);
...
sub call_method {
    ...
    my $ctx = ProbabilisticAnnotationServerContext->new($self->{loggers}->{userlog},
...
{
    # Service ProbabilisticAnnotation requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for ProbabilisticAnnotation but no authentication header was passed");
	}
	...
	my $stderr = ProbabilisticAnnotationServerStderrWrapper->new($ctx, $get_time);
	$ctx->stderr($stderr);
...
package ProbabilisticAnnotationServerContext;

use strict;

=head1 NAME

ProbabilisticAnnotationServerContext
...
package ProbabilisticAnnotationServerStderrWrapper;
